...
val dataCols = data.select($"Fresh", $"Milk", $"Grocery", $"Frozen", $"Detergents_Paper", $"Delicassen").as("feature_data")
...
val assembler = new VectorAssembler().setInputCols(Array("feature_data")).setOutputCol("features")
